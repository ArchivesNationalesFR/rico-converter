...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc>
        ...
        <xd:desc>
            ...
            <xd:p>Generating RDF files from the XML files created by the Archives nationales de
                ...
                districts in the main file, FRAN_RI_005.xml. Step 2 : integrating historical data from INSEE datasets + some partitive relations. </xd:p>
    ...
    <xsl:variable name="ANdepts" select="document('rdf/places-step1/FRAN_departements.rdf')/rdf:RDF"/>
    <xsl:variable name="chemin-communes">
        <xsl:value-of
            select="concat('rdf/places-step1/', '?select=FRAN_communes*.rdf;recurse=yes;on-error=warning')"
        />
    </xsl:variable>
    ...
    <xsl:variable name="chemin-arrs">
        <xsl:value-of
            select="concat('rdf/places-step1/', '?select=FRAN_arrondissements*.rdf;recurse=yes;on-error=warning')"
        />
    </xsl:variable>
    ...
    <xsl:variable name="chemin-cantons"
        select="concat('rdf/places-step1/', '?select=FRAN_cantons*.rdf;recurse=yes;on-error=warning')"/>
    <xsl:variable name="cantons" select="collection($chemin-cantons)"/>
    
    <xsl:variable name="INSEEData"
        select="document('src/RI/places/sparql-graphdb-mouvementsCommunes.xml')/sparql:sparql/sparql:results"/>
    <xsl:variable name="INSEEDataArrs"
        select="document('src/RI/places/sparql-graphdb-mouvementsArrondissements.xml')/sparql:sparql/sparql:results"/>
    <xsl:variable name="INSEEDataDepts"
        select="document('src/RI/places/sparql-graphdb-mouvementsDepartements-2.xml')/sparql:sparql/sparql:results"/>
    ...
    <xsl:template match="/vide">
        ...
        <xsl:for-each select="$communes">
            <xsl:variable name="deptId">
                <xsl:choose>
                    <xsl:when
                        test="starts-with(/rdf:RDF/rdf:Description[1]/insee-geo:codeCommune, '97') or starts-with(/rdf:RDF/rdf:Description[1]/insee-geo:codeCommune, '98')">
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeCommune, 1, 3)"
                        />
                    </xsl:when>
                    <!-- <xsl:otherwise><xsl:value-of select="generate-id()"/></xsl:otherwise>-->
                    <xsl:otherwise>
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeCommune, 1, 2)"
                        />
                    </xsl:otherwise>
                </xsl:choose>
        ...
        <xsl:for-each select="$arrs">
            <xsl:variable name="deptId">
                <xsl:choose>
                    <xsl:when
                        test="starts-with(/rdf:RDF/rdf:Description[1]/insee-geo:codeArrondissement, '97')">
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeArrondissement, 1, 3)"
                        />
                    </xsl:when>
                    <!-- <xsl:otherwise><xsl:value-of select="generate-id()"/></xsl:otherwise>-->
                    <xsl:otherwise>
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeArrondissement, 1, 2)"
                        />
                    </xsl:otherwise>
                </xsl:choose>
        ...
        <xsl:for-each select="$cantons">
            <xsl:variable name="deptId">
                <xsl:choose>
                    <xsl:when
                        test="starts-with(/rdf:RDF/rdf:Description[1]/insee-geo:codeCanton, '97')">
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeCanton, 1, 3)"
                        />
                    </xsl:when>
                    <!-- <xsl:otherwise><xsl:value-of select="generate-id()"/></xsl:otherwise>-->
                    <xsl:otherwise>
                        <xsl:value-of
                            select="substring(/rdf:RDF/rdf:Description[1]/insee-geo:codeCanton, 1, 2)"
                        />
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="rdf:Description">
        ...
        <rdf:Description>
            ...
            <xsl:if
                ...
                <xsl:if test="$arrs/rdf:RDF/rdf:Description[owl:sameAs[@rdf:resource = $idArr]]">
                    <geofla:arr>
                        <xsl:attribute name="rdf:resource">
                            <xsl:value-of
                                select="$arrs/rdf:RDF/rdf:Description[owl:sameAs[@rdf:resource = $idArr]]/@rdf:about"
                            />
                        </xsl:attribute>
                    </geofla:arr>
                    <RiC:isPartOf>
            ...
            <xsl:if test="rdf:type[@rdf:resource = 'http://data.ign.fr/def/geofla#Commune']">
                ...
                <xsl:if test="RiC:note[starts-with(., 'arrondissement :')]">
                    <xsl:variable name="idArr"
                        select="normalize-space(substring-after(RiC:note[starts-with(., 'arrondissement :')], 'arrondissement :'))"/>
                    ...
                    <xsl:if
                        test="$arrs/rdf:RDF/rdf:Description[insee-geo:codeArrondissement = $trueIdArr]">
                        <geofla:arr>
                            <xsl:attribute name="rdf:resource">
                                <xsl:value-of
                                    select="$arrs/rdf:RDF/rdf:Description[insee-geo:codeArrondissement = $trueIdArr]/@rdf:about"
                                />
                            </xsl:attribute>
                        </geofla:arr>
                        <RiC:isPartOf>
                ...
                <xsl:if test="RiC:note[starts-with(., 'canton :')]">
                    <xsl:variable name="CantName"
                        select="normalize-space(substring-after(RiC:note[starts-with(., 'canton :')], 'canton :'))"/>

                    <xsl:if
                        test="$cantons/rdf:RDF/rdf:Description[starts-with(rdfs:label, concat($CantName, ' ('))]">
                        <geofla:cant>
                            <xsl:attribute name="rdf:resource">
                                <xsl:value-of
                                    select="$cantons/rdf:RDF/rdf:Description[starts-with(rdfs:label, concat($CantName, ' ('))]/@rdf:about"
                                />
                            </xsl:attribute>
                        </geofla:cant>
                        <RiC:isPartOf>
                ...
                <xsl:if test="$INSEEdataByCommunes/rdf:INSEEdata/rdf:commune[@uri = $INSEEUri]">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when
                            ...
                            <RiC:state xml:lang="fr">commune disparue</RiC:state>
                        </xsl:when>
            ...
            <xsl:if test="$INSEEdataByArrs/rdf:INSEEdata/rdf:arrondissement[@uri = $INSEEUri]">
                <xsl:choose>
                    ...
                    <xsl:when
                        test="not(RiC:state) and $INSEEdataByArrs/rdf:INSEEdata/rdf:arrondissement[@uri = $INSEEUri]/@etat = 'false'">
                        <RiC:state xml:lang="fr">arrondissement disparu</RiC:state>
                    </xsl:when>
            ...
            <xsl:if
                ...
                <xsl:if
                    ...
                    <RiC:state xml:lang="fr">département disparu</RiC:state>
                </xsl:if>
                    <xsl:if

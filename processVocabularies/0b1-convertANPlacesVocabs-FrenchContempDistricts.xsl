...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="insee-arrs"
        select="document('src/places/SPARQL-datalift-arrondissements-INSEE.xml')/sparql:sparql"/>
    <xsl:variable name="insee-cants"
        select="document('src/places/SPARQL-datalift-cantonsINSEE.xml')/sparql:sparql"/>
    ...
    <xsl:template match="/r">
        ...
        <xsl:result-document method="xml" encoding="utf-8" indent="yes"
            href="{$step1}FRAN_departements.rdf">
            <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
                ...
                xml:base="http://data.archives-nationales.culture.gouv.fr/"><xsl:for-each
                    select="$departs/d">
                    <xsl:sort select="terme"/>
                    <xsl:variable name="id" select="@id"/> <xsl:variable name="INSEEId"
                        select="
                    ...
                    <xsl:variable name="regionName">
                        ...
                        </xsl:choose> </xsl:variable> <xsl:variable name="regionId">
                        <xsl:choose>
                    ...
                    </xsl:variable> <rico:Place>
                        <xsl:attribute name="rdf:about">
                            <xsl:value-of select="concat('place/', $ANVocabId, '-', $id)"/> </xsl:attribute>
                        ...
                        <rico:hasPlaceType rdf:resource="placeType/d%C3%A9partement"/>
                            ...
                            <xsl:choose>
                                <xsl:when test="contains(terme, '(France')">
                                    <xsl:variable name="date">
                                        ...
                                        select="substring-before($date, '-')"/> </xsl:when>
                                <xsl:when
                                ...
                                <xsl:otherwise> <xsl:text>1790</xsl:text>
                                </xsl:otherwise>
        ...
        igeo:vivant       true .-->
                            ...
                            <rico:endDate rdf:datatype="http://www.w3.org/2001/XMLSchema#gYear">
                                <xsl:variable name="date">
                                    <xsl:value-of
                                        select="substring-before(normalize-space(substring-after(terme, 'département ,')), ')')"
                                    />
                                </xsl:variable> <xsl:value-of select="substring-after($date, '-')"/>
                            </rico:endDate>
                        </xsl:if>
                        <xsl:choose>
                            <xsl:when
                        ...
                        <xsl:if test="$regionId != 'non'">
                            <rico:isPartOf>
                                <xsl:attribute name="rdf:resource"> <xsl:value-of
                                        select="concat('place/', $ANVocabId, '-', $regionId)"/>
                                </xsl:attribute>
                            </rico:isPartOf>
                            <geofla:region>
                                <xsl:attribute name="rdf:resource"> <xsl:value-of
                ...
                <rico:Place rdf:about="place/FRAN_RI_005-d5bdp7f563-3i98jv8c3b0g-bis">
                    ...
                    <rico:followsInTime rdf:resource="place/FRAN_RI_005-d5bdp7f563-3i98jv8c3b0g"/>
                    <!--  <RiC:hasPlaceName
  ...
  <d id="d3ntcnfgcw-pvic5jinawzd">
   ...
   <noticel>
    ...
    <insee>01001</insee>
        ...
        <xsl:for-each-group select="$communes/d" group-by="noticel/dpt">
            ...
            <xsl:result-document method="xml" encoding="utf-8" indent="yes"
                href="{$step1}FRAN_communes_{$INSEEdeptId}.rdf">
                <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
                    xmlns:owl="http://www.w3.org/2002/07/owl#"
                    xmlns:rico="http://www.ica.org/standards/RiC/ontology#"
                    xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#"
                    ...
                    <xsl:for-each select="current-group()">
                        <rico:Place>
                            ...
                            <rico:hasPlaceType rdf:resource="placeType/commune"/>
                            <rico:identifier xml:lang="fr">
                                <xsl:value-of
                                    select="concat('Ancien identifiant SIA : ', $ANVocabId, '.xml#', @id)"
                                />
                            </rico:identifier>
                            ...
                            <rico:isPartOf>
                                <xsl:attribute name="rdf:resource">
                                    <xsl:value-of
                                        select="concat('place/', $ANVocabId, '-', $SIAdeptId)"/>
                                </xsl:attribute>
                        ...
                        <!-- traitement des lieux subordonnés aux communes dans le RI_005 -->
                        <xsl:for-each select="d">
                            <rico:Place>
                                <xsl:attribute name="rdf:about">
                                    <xsl:value-of select="concat('place/FRAN_RI_005-', @id)"/>
                                </xsl:attribute>
                                <rdfs:label xml:lang="fr">
                                    <xsl:value-of select="normalize-space(terme)"/>
                                </rdfs:label>
                                ...
                                <xsl:for-each select="notice1/forms/f">
                                    <skos:altLabel xml:lang="fr">
                                        <xsl:value-of select="normalize-space(.)"/>
                                    </skos:altLabel>
                                </xsl:for-each>
                                ...
                                <xsl:variable name="type"
                                    select="normalize-space(substring-before(substring-after(def, 'Type :'), '|'))"/>
                                <xsl:choose>
                                    <xsl:when test="$type = 'hameau'">
                                        <rico:hasPlaceType rdf:resource="placeType/hameau"/>

                                    </xsl:when>
                                    <xsl:when test="$type = 'quartier'">
                                        <rico:hasPlaceType rdf:resource="placeType/quartier"/>

                                    </xsl:when>
                                    <xsl:when test="$type = 'village'">
                                        <rico:hasPlaceType rdf:resource="placeType/village"/>

                                    </xsl:when>
                                    <xsl:when test="$type = 'lieu-dit'">
                                        <rico:hasPlaceType rdf:resource="placeType/lieu-dit"/>

                                    </xsl:when>
                                    <xsl:when test="$type = 'ancienne commune'">
                                        <rico:hasPlaceType
                                            rdf:resource="placeType/ancienne%20commune"/>

                                    </xsl:when>
                                    <xsl:when test="$type = 'bourg'">
                                        <rico:hasPlaceType rdf:resource="placeType/bourg"/>

                                    </xsl:when>
                                </xsl:choose>
                                <xsl:if test="$type != 'ancienne commune'">
                                    <rico:isPartOf>
                                        <xsl:attribute name="rdf:resource">
                                            <xsl:value-of
                                                select="concat('place/', $ANVocabId, '-', parent::d/@id)"
                                            />
                                        </xsl:attribute>
                                    </rico:isPartOf>
                                </xsl:if>
                                ...
                                <xsl:for-each
                                    select="tokenize($def, '\|')[normalize-space(.) != '']">

                                    <xsl:choose>
                                        <xsl:when test="starts-with(normalize-space(.), 'Type')"/>
                                        <xsl:when test="starts-with(., 'GeoNames ')">
   ...
   il y a aussi des : circonscription teritoriale, district administratif, subdivision administrative (16)
        ...
        <xsl:for-each-group select="$arrts/d"
            ...
            <xsl:result-document method="xml" encoding="utf-8" indent="yes"
                href="{$step1}FRAN_arrondissements_{$INSEEdeptId}.rdf">
                <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
                    xmlns:owl="http://www.w3.org/2002/07/owl#"
                    xmlns:rico="http://www.ica.org/standards/RiC/ontology#"
                    xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#"
                    ...
                    <xsl:for-each select="current-group()">
                        ...
                        <rico:Place>
                            ...
                            <rico:hasPlaceType rdf:resource="placeType/arrondissement"/>
                            <rico:identifier xml:lang="fr">
                                <xsl:value-of
                                    select="concat('Ancien identifiant SIA : ', $ANVocabId, '.xml#', @id)"
                                />
...
-->
   ...
   <terme>Abbeville-Nord (Somme , canton)</terme>
        ...
        <xsl:for-each-group select="$cants/d"
            ...
            if (terme='Bastia  5e  (Canton Lupino) (Haute-Corse, canton)'
            or terme='Bastia 6e (Canton Furiani-Montésoro) (Haute-Corse, canton)')
        then (
       'Haute-Corse' 
        
        )
        else (normalize-space(replace(substring-before(substring-after(terme, '('), ', canton'), '&#xa0;', '')))
            ...
            <!-- <xsl:if test="$INSEEdeptId!='non'">-->
            <xsl:result-document method="xml" encoding="utf-8" indent="yes"
                href="{$step1}FRAN_cantons_{$INSEEdeptId}.rdf">
                <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
                    xmlns:owl="http://www.w3.org/2002/07/owl#"
                    xmlns:rico="http://www.ica.org/standards/RiC/ontology#"
                    xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#"
                    ...
                    <xsl:for-each select="current-group()">
                        ...
                        <rico:Place>
                            ...
                            <rico:hasPlaceType rdf:resource="placeType/canton"/>
                            <rico:identifier xml:lang="fr">
                                <xsl:value-of
                                    select="concat('Ancien identifiant SIA : ', $ANVocabId, '.xml#', @id)"
                                />
